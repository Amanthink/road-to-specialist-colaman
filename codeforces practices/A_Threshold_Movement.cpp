...
void solve(){
    ...
    ll minOdd = inf;   // 1-based odd positions: 1, 3, 5, ...
    ll maxEven = -inf;  // 1-based even positions: 2, 4, 6, ...

    for (int i = 0; i < n; i++) {
        if (i % 2 == 0) minOdd = min(minOdd, a[i]);
        else maxEven = max(maxEven, a[i]);
    }

    cout << (maxEven + 1 < minOdd ? "YES\n" : "NO\n");
}
