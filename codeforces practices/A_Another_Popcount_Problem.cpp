...
void solve(){
    ...
    int n, k;
        ...
        for (int i = 1; i <= n; i<<= 1) {// i is a power of 2
            int add = min(k, n / i);// add is the number of times we can use this power of 2 without exceeding k or n/i
            ans += add;// add to the answer
            n -= add * i;
        }
