...
void solve(){
    ...
    vi a(n);
    for(i,0,n)cin>>a[i];
    //maximum difference between endpoints of the array
    ...
    //check the maximum difference between the first element and any other element
    for (i,0,n) {
        answer = max(answer, a[i] - a[0]);
    }
    //check for two consecutive elements in the array and find the maximum difference between them
    for (i,0,n-1) {
